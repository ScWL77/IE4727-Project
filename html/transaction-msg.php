...
<html>
    <head>
        <title>Transaction Message Page</title>
        <meta charset="utf-8">
        ...
        <div class="container">
            <div class="msg-box">
                <div class="icon-container">
                    <?php
                        if (isset($_GET['status']) && $_GET['status'] === 'success') {
                            echo '<i class="fa fa-check-circle-o check"></i>'; // Success icon
                        } else {
                ...
                <div class="msg-container">
                    <?php
                        if (isset($_GET['status']) && $_GET['status'] === 'success') {
                            ...
                            echo '<b>Payment Error!</b>';
                        }
                    ?>
                </div>
                <div class="subMsg-container">
                    <?php
                        if (isset($_GET['status']) && $_GET['status'] === 'success') {
                            echo '<p id="subMsg">Your transaction summary has been <br> sent to your email.</p>';
                        } elseif(isset($_GET['status']) && $_GET['status'] === 'booking_clash') {
                            echo '<p id="subMsg">There are clashes in timing with your current bookings. <br> Please try again!</p>';
                        } elseif(isset($_GET['status']) && $_GET['status'] === 'past_clash')
                        {
                            echo '<p id="subMsg">There are clashes in timing with your past transacted bookings. <br> Please try again!</p>';
                        }elseif(isset($_GET['status']) && $_GET['status'] === 'both')
                        {
                            echo '<p id="subMsg">There are clashes in timing with both your current bookings and with your past transacted bookings. <br> Please try again!</p>';
                        }else{
                            echo '<p id="subMsg">There was an error processing your transaction. <br> Please try again!</p>';
                        }
                    ?>
                </div>
                <div class="next-button">
                    <?php
                        if (isset($_GET['status']) && $_GET['status'] === 'success') {
                            echo '<button onclick="window.location.href=\'home.php\'">Back to Home</button>';
                        } else {
                            echo '<button onclick="window.location.href=\'cart.php\'">Back to Bookings</button>';
                        }
                    ?>
                </div>
            </div>
